...
while [[ $? -eq 0 ]]; do
	...
	# Output the line.
	if [[ "$cls" ]]; then
		currSection+='<div class="'${cls}'">'${s}'</div>'
	else
		currSection+='<div>'${s}'</div>'
	fi
